...
const pages = [
  ({ style }) => (
    <animated.div style={{ ...style, background: '#2D4159' }}>
      <Home />
    </animated.div>
  ),
  ({ style }) => (
    <animated.div style={{ ...style, background: '#ccc' }}>
      <About />
    </animated.div>
  ), //'#279761'
  ({ style }) => (
    <animated.div style={{ ...style, background: '#CEBC81' }}>
      <Portfolio />
    </animated.div>
  ),
  ({ style }) => (
    <animated.div style={{ ...style, background: '#A16E83' }}>
      Building it.. Sorry for the inconvenience!
    </animated.div>
  ),
  ({ style }) => (
    <animated.div style={{ ...style, background: '#B19F96' }}>
      Building it.. Sorry for the inconvenience!
    </animated.div>
  ),
  ({ style }) => (
